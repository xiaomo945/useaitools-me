...
- **Value** 填：一个收款页链接，比如 Ko-fi / PayPal.Me / Polar 的页面地址
- **类型**：选 **配置**
- **Environments**：三个都勾选

> 这个**不填也能跑**——不填的话，页面上给客户显示的就是"直接发邮件到上面的收款邮箱"。代码里 `SPONSOR_PAYMENT_URL` 的默认值是空字符串，且不参与任何校验，所以留空不会报错。
> 填了的话，页面上就变成一个"去付款页"的按钮，客户点一下就能付，你少收一封邮件。等你回头建好收款页，随时可以回来加。

**如果想用 PayPal，先看这里（重要）**

最容易拿到的是 **PayPal.Me 收款链接**，创建路径：登录 `https://www.paypal.com` → 右上角头像 → 找到 **PayPal.Me**（或在账户设置 → 收款里）→ 创建一个链接 → 得到形如 `https://paypal.me/你的用户名`。

但**先确认一件事**：如果你注册的是**中国大陆手机号**绑的 PayPal 中国账户（走财付通那套），这类账户**通常只能付款、不能接收境外付款**。

后果是：客户老老实实付了 $150 → 钱被退回或卡住 → 订单永远停在 pending → 你钱收不到还得道歉。**比不放链接更糟。**

怎么确认：拿这个账户试着给自己发 1 美元，看能否正常到账。不确定的话，这个变量就先空着，用上面的收款邮箱兜底。

**替代方案（对卖广告位给海外独立开发者这个场景更合适）**：

- **Ko-fi**（https://ko-fi.com）：海外 indie / AI 圈子用得最多，免费创建，支持绑定你自己的卡收款
- **Polar**（https://polar.sh）：面向开发者，支持国际卡

一个细节：代码**不会**自动把金额拼到收款页地址后面。客户点进去要自己输 $150。所以创建收款链接时，尽量把金额预设好，或者在页面上写明金额。

#### 变量 4（可选，但有它才专业）：`RESEND_API_KEY`

在 https://resend.com 用 GitHub 登录，免费额度足够。Dashboard → API Keys → Create API Key，复制那串以 `re_` 开头的密钥。
...
**为什么值得配**：客户付了钱、你把他的广告位点成「已上架」之后，系统会给客户发一封确认邮件。配了这串密钥，邮件真的能到客户邮箱；不配的话**邮件根本发不出去**。

不配也能用，但后台上架时那一条订单上会弹出黄色提示，告诉你"客户没有被发邮件，需要你手动联系"。这只是让你别忘，不影响上架。

> 顺带一提：付费收录的收款目前走的是"客户转账 → 你收到邮件 → 后台点确认"。也就是说，**收款邮箱本身就是最重要的一个变量**——它和 `RESEND_API_KEY` 配好之后，整条链路才算闭环。
