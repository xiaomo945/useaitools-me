...
export default function ToolDetailClient({ tool, relatedTools, relatedArticles = [] }: { tool: Tool; relatedTools: Tool[]; relatedArticles?: BlogPost[] }) {
  ...
  const ctaText = hasAffiliate ? 'Try It Free — Special Deal' : 'Visit Website';
            ...
            <div className="flex flex-wrap gap-3">
              {hasAffiliate && (
                <div className="w-full flex items-center gap-2 mb-1">
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 text-xs font-semibold border border-emerald-200 dark:border-emerald-500/20">
                    <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                    Exclusive Deal
                  </span>
                  <span className="text-xs text-slate-400 dark:text-gray-500">Limited time offer</span>
                </div>
              )}
              <a
                href={ctaUrl}
