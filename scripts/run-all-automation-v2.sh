...
run_batch_content() {
    ...
    local batch_script="$SCRIPT_DIR/batch-content-producer-v2.js"
...
#-------------------------------------------------------------------------------
# Task 6: GSC Auto-Submit
#-------------------------------------------------------------------------------

run_gsc_submit() {
    log_section "TASK 6: GSC AUTO-SUBMIT"
    
    local gsc_script="$SCRIPT_DIR/auto-submit-gsc.py"
    
    if [ ! -f "$gsc_script" ]; then
        log_warn "⚠ GSC script not found: $gsc_script"
        return 0
    fi
    ...
    if check_command python3; then
        if run_task "Submit new content to GSC" "python3 '$gsc_script' --days 1" 2; then
            ...
            log_warn "⚠ GSC submission failed - continuing"
            return 0
        fi
    else
        log_warn "⚠ Python3 not available - skipping GSC submission"
        return 0
    fi
}

#-------------------------------------------------------------------------------
# Task 7: Internal Links
#-------------------------------------------------------------------------------

run_internal_links() {
    log_section "TASK 7: INTERNAL LINKS UPDATE"
...
#-------------------------------------------------------------------------------
# Task 8: Affiliate Links Check
#-------------------------------------------------------------------------------

run_affiliate_check() {
    log_section "TASK 8: AFFILIATE LINKS CHECK"
...
#-------------------------------------------------------------------------------
# Task 9: External URL Validation
#-------------------------------------------------------------------------------

run_url_check() {
    log_section "TASK 9: EXTERNAL URL VALIDATION"
...
generate_summary() {
    ...
    if [ -d "$PROJECT_DIR/.tmp" ]; then
        ...
        if [ -f "$PROJECT_DIR/.tmp/gsc-submit-log.md" ]; then
            echo "  ✓ GSC log: .tmp/gsc-submit-log.md"
        fi
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --help)
                ...
                echo "  --gsc-only        Run only GSC auto-submit"
                ...
                echo "  --skip-gsc        Skip GSC auto-submit"
    ...
    if [ "$RUN_GSC" = true ]; then
        if ! run_gsc_submit; then
            failed_tasks+=("GSC Submit")
        fi
    fi
